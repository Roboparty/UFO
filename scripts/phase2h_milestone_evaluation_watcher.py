#!/usr/bin/env python3
"""Evaluate the no-odometry Actor baseline and Phase 2H milestones asynchronously."""
...
def _run_parallel(jobs: list[tuple[list[str], Path]], gpus: tuple[int, ...]) -> None:
    """Run at most one evaluation per GPU at a time."""
    for offset in range(0, len(jobs), len(gpus)):
        processes: list[tuple[subprocess.Popen, object, Path]] = []
        for gpu, (command, log_path) in zip(gpus, jobs[offset : offset + len(gpus)], strict=False):
...
def _evaluate_stairs(args, *, actor: Path, output: Path) -> None:
    jobs = []
...
def _write_comparison(args, labels: list[tuple[str, Path]]) -> None:
    stairs_rows: list[dict[str, object]] = []
    regression_rows: list[dict[str, object]] = []
    for checkpoint, root in labels:
        stairs_path = root / "aggregate" / "step_height_ci.csv"
        if stairs_path.exists():
            with stairs_path.open(newline="") as stream:
                for row in csv.DictReader(stream):
                    if row["mode"] == "temporal" and float(row["step_height_m"]) >= 0.14:
                        stairs_rows.append({"checkpoint": checkpoint, **row})
        for terrain in REGRESSION_TERRAINS:
            path = root / "regression" / terrain / "summary.json"
            if not path.exists():
                continue
            temporal = json.loads(path.read_text())["modes"]["temporal"]
            regression_rows.append(
                {
                    ...
                    "terrain_input_mae": temporal["terrain_input_mae_mean"],
                }
            )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--baseline-actor", type=Path, required=True)
    ...
    parser.add_argument("--gpus", type=int, nargs="+", default=[1, 2, 3, 4, 5, 6, 7])
...
def main() -> None:
    ...
    baseline_output = args.output_root / "baseline_actor2000"
    _evaluate_stairs(args, actor=args.baseline_actor, output=baseline_output)
    _evaluate_regressions(args, actor=args.baseline_actor, output=baseline_output)
    completed.append(("baseline_actor2000", baseline_output))
    _write_comparison(args, completed)
    ...
    for step in args.steps:
        ...
        _evaluate_stairs(args, actor=actor, output=output)
        _evaluate_regressions(args, actor=actor, output=output)
        completed.append((str(step), output))
        _write_comparison(args, completed)
        (args.output_root / "progress.json").write_text(json.dumps({"last_completed_step": step, "steps": args.steps}, indent=2) + "\n")
