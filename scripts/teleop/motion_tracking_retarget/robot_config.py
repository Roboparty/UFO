...
from .params import SUPPORTED_ROBOTS, resolve_robot_xml_path

PACKAGE_ROOT = Path(__file__).resolve().parent
UFO_ROOT = PACKAGE_ROOT.parents[2]
CONFIG_ROOT = UFO_ROOT / "config" / "teleop"
...
def load_teleop_robot_config(robot: str) -> TeleopRobotConfig:
    ...
    cfg_path = CONFIG_ROOT / f"{robot_key}.yaml"
    ...
    if not recorder_output_dir.is_absolute():
        recorder_output_dir = UFO_ROOT / recorder_output_dir
